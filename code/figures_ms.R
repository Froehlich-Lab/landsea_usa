...
setwd("~/Dropbox/FROEHLICH LAB/Projects/TNC Cons Aqau/PollFish_results")
...
prun.rf <- read_csv("Pruned_model_results.csv")
head(full.rf)
...
p2<-ggplot(data, aes(x = Q4_fish_hunt, y = Q7_impact_me))+
...
setwd("~/Dropbox/FROEHLICH LAB/Projects/TNC Cons Aqau/PollFish_results")
data <- read_csv("LandvSea_Q4.csv")
head(data)


freq_data <- table(data$activity, data$binary)
freq_data <- as.data.frame(freq_data)
names(freq_data)<- c("act", "response", "freq")
freq_data
levels(freq_data$act)
freq_data$act <- ordered(freq_data$act ,
                         levels = c("camping", "hiking", "fishing"
                                    ,"sailing", "snorkeling/diving", "hunting", "none"))

p3<-ggplot(freq_data, aes(x = act, y = freq, fill= response))+
  labs(x = '', y = "Frequency of Responses")+ 
  geom_bar(stat = "identity")+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        axis.text.x  = element_text(size=11),axis.text.y  = element_text(size=11),
